...
function Blog() {
  ...
  return (
    ...
    <div className="flex justify-end items-center h-1/5">
        <button onClick={(e)=>{handleblogsubmit(e);dispatch(fetchBlog())}} type= 'submit'>Save</button>
    ...
    {email === blog.email?
     ...
     <button className='bg-green-500 h-full' id="update" onClick={()=>{setBlogid(blog.blog_id);setTitle(blog.blog_detail.title ); setDescription(blog.blog_detail.description); setTime(blog.blog_detail.time);modal.showModal(); }}>Update</button>
