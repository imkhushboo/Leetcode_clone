...
function Blog() {
  ...
  return (
    <div id="blog" className='mt-[5%] m-auto w-[98%] h-[80vh] overflow-y-contain'>
        ...
        <div className='flex flex-col h-2/5 justify-between'>
            ...
            <textarea className='h-2/5 resize-none' row='10' col='10'value={profile.description} onChange={(e)=>{setProfile({...profile,'time':Date(),'description': e.target.value})}}> </textarea>
        </div>
        <hr />
        ...
        {profile.email === blog.email?
         ...
         <button className='bg-green-500 h-full' id="update" onClick={()=>{rendermodal({title : blog.blog_detail.title , description :blog.blog_detail.description ,time:blog.blog_detail.time ,blogid:blog.blog_id}) }}>Update</button>
         <button className='bg-gray-500 h-full'id="delete" onClick={() => {deleteblog(blog.blog_id)}}>Delete</button>
      </div>:null}
